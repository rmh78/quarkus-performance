...
jabba ls | while read CURRENT_JAVA; do
    ...
    ./test-single.sh "java -Xmn128M -Xmx512M -jar demo-payara/target/demo-payara-microbundle.jar --noCluster" $DEMO_URL payara-micro-$CURRENT_JAVA "Payara Micro (5.193) via Java Runtime ($CURRENT_JAVA)"
    sleep 2
